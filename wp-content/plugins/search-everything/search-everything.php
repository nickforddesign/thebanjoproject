...
class SearchEverything {
	...
	function se_search_default(){
		...
		foreach ( $terms as $term ) {
			...
			// LOL u mad bro? this hack prevent the title and content from being searched, date field will never match but it prevents the query syntax from breaking
			$like_date = "($wpdb->posts.post_date LIKE '$esc_term')";

			$search_sql_query .= "($like_date)";
